...
namespace webrtc
{
	...
	NvPipeEncoderH264::NvPipeEncoderH264()
		: max_payload_size_(0)
	...
	int32_t NvPipeEncoderH264::InitEncode(const VideoCodec* codec_settings, int32_t number_of_cores, size_t max_payload_size)
	{
		ReportInit();
	...
	int32_t NvPipeEncoderH264::Release()
	{
		if (encoder_)
		{
			auto& nvPipe = getNvPipe();
	...
	int32_t NvPipeEncoderH264::RegisterEncodeCompleteCallback(
	...
	int32_t NvPipeEncoderH264::SetRateAllocation(const VideoBitrateAllocation& bitrate, uint32_t new_framerate)
	{
		if (!encoder_)
	...
	int32_t NvPipeEncoderH264::Encode(const VideoFrame& input_frame,
		...
		const std::vector<FrameType>* frame_types)
	{
		...
		if (is_sending_ && (frame_types == nullptr || frame_types->at(0) != kEmptyFrame))
		{
			...
			case VideoFrameFormat::GpuTextureD3D11:
			{
				...
				ID3D10Multithread* lock;
				ctx->QueryInterface<ID3D10Multithread>(&lock);
				printf("Enter H264 lock\n");
				lock->Enter();
	...
	void NvPipeEncoderH264::ReportInit()
	{
		if (has_reported_init_)
			return;
		RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.NvPipeEncoderH264.Event",
			kH264EncoderEventInit, kH264EncoderEventMax);
		has_reported_init_ = true;
	}

	void NvPipeEncoderH264::ReportError()
	{
	...
	VideoEncoder::EncoderInfo NvPipeEncoderH264::GetEncoderInfo() const
	{
	...
	void NvPipeEncoderH264::SetStreamState(bool send_stream)
	{
	...
	bool NvPipeEncoderH264::IsAvailable()
	{
