...
namespace webrtc {
    ...
    void FrameGeneratorCapturer::InsertFrame() {
        ...
        if (sending_) {
            ...
            const int decimation = static_cast<int>(std::round(static_cast<double>(source_fps_) / target_capture_fps_));
    ...
    void FrameGeneratorCapturer::ChangeFramerate(int target_framerate) {
        ...
        if (source_fps_ % target_capture_fps_ != 0) {
            const int decimation = static_cast<int>(std::round(static_cast<double>(source_fps_) / target_capture_fps_));
            const int effective_rate = target_capture_fps_ / decimation;
